...
% ------------------- START OF YOUR PICTURE --------------------
  ...
  % (Removed manual coordinate definitions)
    ...
    \foreach \i [evaluate=\i as \j using int(\i+1)] in {0,1} {
     ...
     \draw[thr]  (S\i.east) -- node[labelthr]  {$-U_{\mathrm{thr}}$} (U\j.north west);
    }

     % Input weights between successive columns
      % \draw[weights] (-1.55,-1.25) -- node[labelweights] {$W_{-1}$} (I0.south west);
      \draw[weights] (X0.north east) -- node[labelweights] {$W_{-1}$} (I1.south west);
      \draw[weights] (X1.north east) -- node[labelweights] {$W_{-1}$} (I2.south west);
      % \draw[weights] (X2.north east) -- ++(\blueRun,\blueRise);
