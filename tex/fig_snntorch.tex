...
\usepackage{tikz}
\usetikzlibrary{arrows.meta,calc,positioning,shapes,decorations.pathreplacing} % etc.
...
% ------------------- START OF YOUR PICTURE --------------------
  ...
  \node[font=\Large] at (\ellipsisX,-2.) {$\cdots$};
  \node[font=\Large] at (\ellipsisX,-.5) {$\cdots$};
  \node[font=\Large] at (\ellipsisX,1.0) {$\cdots$};
  \node[font=\Large] at (\ellipsisX,2.5) {$\cdots$};
