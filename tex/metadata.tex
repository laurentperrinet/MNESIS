%!TeX encoding = UTF-8 Unicode
%!TeX spellcheck = en-UK
% !TeX root = Perrinet26mnesis.tex
% -*- coding: UTF-8; -*-
\newcommand{\Title}{Working Memory with Polychronous Chains}
\newcommand{\FirstLP}{Laurent U}
\newcommand{\LastLP}{Perrinet}
\newcommand{\AuthorLP}{\FirstLP\ \LastLP}
\newcommand{\EmailLP}{laurent.perrinet@univ-amu.fr}
\newcommand{\orcidLP}{0000-0002-9536-010X}
\newcommand{\Department}{Institut de Neurosciences de la Timone (UMR 7289)}%
\newcommand{\Affiliation}{Aix Marseille Univ, CNRS}%
\newcommand{\Street}{27 boulevard Jean Moulin}%
\newcommand{\PostCode}{13005}%
\newcommand{\City}{Marseille}%
\newcommand{\Country}{France}%
\newcommand{\WebsiteLP}{https://laurentperrinet.github.io}%
\newcommand{\Acknowledgements}{
This work was performed using HPC resources from GENCI-IDRIS (Grant 2025--AD010314955R2).
The authors declare no competing interests regarding this manuscript. The funders had no role in the design of the study; in the collection, analyses, or interpretation of data; in the writing of the manuscript; or in the decision to publish the results.
For the purpose of open access, the authors have applied a CC-BY public copyright licence to any Author-Accepted Manuscript version arising from this submission, and the different versions are available on arXiv at \url{https://arxiv.org/abs/2604.14096}.
}
\newcommand{\AIstatement}{
During the preparation of this paper, I used the Gemma4:31b generative AI tool (\url{https://ollama.com/library/gemma4:31b-mlx}) only for language editing---improving the clarity, grammar, and phrasing of drafted text---and for limited technical assistance with code development. The tool was not used for generating research ideas, formulating research questions or hypotheses, designing the methodology, conducting or interpreting data analysis, or performing mathematical derivations or calculations. All intellectual contributions, analytical work, and mathematical reasoning in this thesis are my own. I have reviewed all AI-assisted content and assume full responsibility for the accuracy and integrity of the entire work.  
}
\newcommand{\Abstract}{%
Working memory --- the ability to store and recall precise temporal patterns of neural activity --- remains an open challenge for spiking neural networks (SNNs). We propose a recurrent SNN of $N=1024$ neurons in which each synapse is equipped with $D = 41$ heterogeneous delays, parameterised as a weight tensor $\mathbf{W} \in \mathbb{R}^{N \times N \times D}$ and trained end-to-end with surrogate-gradient backpropagation through time. Each stored pattern is represented as a sequential chain of overlapping Spiking Motifs: contiguous context windows of length $D$ that uniquely predict the activity at the next time step. A closed-form Hebbian initialisation, derived by deconvolving the LIF membrane response and targeting a sub-threshold membrane potential value superior to the threshold $\vartheta$, achieves an accuracy as measured by the $F_1$-score relative close to that expected when noise is present and before any gradient step on a benchmark of $M=16$ patterns of duration $T=1000\,\mathrm{ms}$. With learning, the network tolerates up to $25\%$ bit-flip noise, and reaches $F_1$ scrores closer to the value expected from the noise level. These results demonstrate attractor-like retrieval dynamics consistent with hippocampal pattern completion, low-dimensional collective dynamics of population activity on manifold or mesoscopic traveling waves in sensory areas. These results show that heterogeneous synaptic delays are an efficient and scalable substrate for working memory in SNNs, with direct implications for neuromorphic edge deployment. %
}
\newcommand{\GeneralPublic}{%
Working memory ---the ability to store and recall precise temporal patterns--- remains an open challenge in computational neuroscience. Recent developments were build as evolutions of the Long-Term Short memory and eventually led to the the adoption of transformers for linking long-range associations, leading to the current AI . However, these models deviated from the biological substrate of the brain : they are humungous and power-hungry.  I will present our recent work on recurrent spiking neural networks (SNNs) and illustrate its application in different fields from neuroscience. This model of working memory is inspired by the structure of information conduction in biological networks and will be based on the dynamics of a realistic yet simple model of single neurons. I will illustrate the insights provided by this model for understanding pattern formation in abstract populations of neurons, projecting high dimensional neural activity on neural manifolds,  modeling travelling waves in sensory areas, or simply storing the text from a book. %
}