...
%%%-----------------------------------------------------------------
To assess working-memory capacity, we generated a dataset of $M = 16$ distinct target patterns $\mathbf{S}^{(\mu)} \in \{0,1\}^{N \times T}$. Following previous work~\citep{Perrinet2023}, each pattern is generated using a stochastic spiking pattern generator with structured and background components. Specifically, for each pattern we define a probability map:
\[
  P^{(\mu)} = p_b \cdot (1 - M^{(\mu)}) + p_s \cdot M^{(\mu)}
\]
where $p_b = 1.6 \times 10^{-4}$ is the background spike probability, $p_s = 0.925$ is the structured spike probability, and $M^{(\mu)} \in \{0,1\}^{N \times T}$ is a binary mask indicating structured regions with sparseness parameter $\alpha = 0.6$. The mask is generated as:
\[
  M^{(\mu)} \sim \mathrm{Bernoulli}(\alpha \cdot p_b / p_s)
\]
The final spike pattern is then drawn as independent Bernoulli trials:
\[
  \mathbf{S}^{(\mu)} \sim \mathrm{Bernoulli}(P^{(\mu)})
\]
This results in an overall mean firing rate of $p_A = 1.6 \times 10^{-4}$ per neuron per time step ($0.16\,\mathrm{Hz}$ for $1\,\mathrm{ms}$ time bins). A refractory period is enforced by imposing a minimum inter-spike interval of $4\,\mathrm{ms}$, yielding biologically plausible sparse activity. 
%%%-----------------------------------------------------------------
