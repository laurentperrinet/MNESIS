...
\begin{equation}
  ...
  \label{eq:init}
\end{equation}
Each weight $w_{i, j, d}$ is proportional to the cross-correlation between the deconvolved spike train of neuron $j$ (scaled by $\vartheta_0 = 0.75$) and the raw spike train of neuron $i$ at delay $d$, averaged over all $M$ patterns. 
...
%%%-----------------------------------------------------------------
The loss is defined as $\mathcal{L} = 1 - F_1$, such that minizing this loss (with a maximum of $1$ and minimum of $0$) maximises the mean $F_1$ score across all $M$ target patterns. Learning is carried out with surrogate-gradient BPTT~\citep{Neftci2019a}, using a fast-sigmoid surrogate with sharpness parameter $\alpha = 5$. Weights are optimised with Adadelta, using a cosine learning-rate schedule with warmup, as used for instance in~\citep{Hammouamri2023}, from a base learning rate $\eta_\mathrm{base} = 40.0 \times 10^{-3}$ to a final learning rate $\eta_\mathrm{final} = 400 \times 10^{-6}$, with $16$ warmup epochs over $256$ total epochs. At each epoch, a new stochastic realisation of the patterns (Eq.~\ref{eq:pattern_generation}) is generated, acting as data augmentation. Dropout regularization with probability $0.60$ is applied to the post-synaptic current. The validation loss is monitored for detecting overfitting.
